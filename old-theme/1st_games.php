...
function convert_png_to_webp($original_image_path, $converted_image_path)
{
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card-price-frame game-page">
          <h2 class="mb-0"><?= Ty::get('walletbalance') ?></h2>
                ...
                <div class="sort-responsive scope_menu">
                  <div class="pos-rel">
                    <div class="d-flex justify-content-center align-items-center tab_game_event mb-10px service-img game-image">
                      <img src="<?= '' . $type_game_template[$key]['img'] ?>">
                    </div>
                  </div>
                  <div class="game-name">
      ...
      <div class="col-md-12">
        <div class="text-white">
          <div class="row">
            <?php foreach ($select_type_game[$type] as $game) {
              // Path to the original image
              $original_image_path = $game['img'];
              $converted_image_file_type = str_replace('.png', '', $original_image_path);
              //  get only file name 
              $converted_image_file_type = explode('/', $converted_image_file_type);
              $converted_image_file_type = end($converted_image_file_type);
              $converted_image_path = 'assets/images/webp/' . $game['code'] . '.webp';
              //  call if not img webp
              if (!file_exists($converted_image_path)) {
                convert_png_to_webp($original_image_path, $converted_image_path);
              }
            ?>
                ...
                <div class="w-100 d-flex justify-content-center">
                  <div class="game-list-img even_game_lists cursor-pointer" product_id="<?= $game['product_id'] ?>" game_type="<?= $game['type'] ?>" game_code="<?= $game['code'] ?>">
                    ...
                    <?php if (file_exists($converted_image_path)) { ?>
                      <img src="<?= $converted_image_path ?>" class="img-responsive">
                    <?php } else { ?>
