...
$call_api = nga_management::selectRandomCardHistory($code, $where, $options);
...
<tbody data-total_count="<?= $total_count ?>">
  <?php foreach ($list as $game_card_history) { ?>
    <tr>
      <td class="text-white"><?= Aww::formatDate($game_card_history['insert_date_time'], 'd/m/Y, H:i'); ?></td>
      <td class="text-white text-nowrap">
        ...
        ?>
          <?= number_format($game_card_history['recive_amount'], 2) . ' ' . $unit; ?>
        <?php } ?>
      </td>
      <td class="text-end">
        <?php if ($game_card_history['status'] == 'confirm') {
