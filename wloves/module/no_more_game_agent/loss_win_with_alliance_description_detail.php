...
<body class="<?= Structure::getThemeClass(); ?>">
  ...
  <div class='bg-whites pb-10px mb-10px'>
    <div class="d-flex top-tap justify-content-between  pt-10px px-15px">
      <div class="msg ">
        <div class='topic'>ยอดแพ้ - ชนะพันธมิตร</div>
        <div class="font-14px text-sub">
          ข้อมูลรายละเอียดยอดแพ้ - ชนะพันธมิตร
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="form-row px-15px ">
      ...
      <div class="col-lg-3">
        <div class="mb-10px">
          ...
          <div class="card-white px-15px py-10px font-Medium">
            <div class=" font-14px">
              <span class="font-20px font-Bold text-primary"> <?= isset($data['count_deposit_first_time']) ? number_format($data['count_deposit_first_time']) : '0'; ?> </span> คน
              <div class="pt-5px">
                ฝากเงินแล้ว <span class="text-success"> <?= isset($data['count_deposit_first_time']) ? number_format($data['count_deposit_first_time']) : '0'; ?> </span>คน คิดเป็น <span class="text-success"> <?= number_format($detail_ally['count_deposit_first_time_percent']); ?>%</span>
              </div>
            </div>
          </div>
        </div>
  ...
  <div id="loss_win_with_alliance_description_detail" class="container-pagination bg-white  no-border-radius" <?= Homepagify::createHomepagify('loss_win_with_alliance_description_detail', '?c=' . $code . '&id=' . $id . '&date_data=' . $date_data, '', 'รายการ') ?>>
      ...
      <table class="table table-sort table-search ">
        <thead>
          <tr>
            ...
            <th nowrap>ยอดฝากทั้งหมด</th>
            <th nowrap>ยอดเงินได้-เสีย</th>
