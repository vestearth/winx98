...
<div class='form-row'>
  <div class="col-lg-5 border-right pb-10px table-mx-0 px-0 bg-whites ">
    <div id="invite_friends_new" class="container-pagination mx-15px no-border-radius no-header" <?= Homepagify::createHomepagify('invite_friends_new', '?c=' . $code . '&user_id=' . $customer_data['id'] . '&friend_id=' . $friend_id, '', 'ลูกค้า') ?>>
      <div class="table-responsive">
        <table class="table table-sort table-search">
          <thead>
            <tr>
              <th nowrap data-sort="username" data-filter="<?= Homepagify::dataFilter('username', 'text') ?>">ยูสเซอร์ (agent)</th>
              <th nowrap data-sort="">ยอดรับรวมสามชั้น</th>
  ...
  <?php
  if ($friend_id) {
    $friend_data = nga_user::getUserDownlineCommissionNew($code, $friend_id, $upline_id);
  ?>
    <div class=" col-lg-7 ">
          ...
          <table class="table p-100 table-style-1 table-border-side mb-0">
            ...
            ?>
              <tbody>
                ...
                ?>
                  <tr>
                    <td><?= Aww::formatDate($played_list['commission_date'], 'd/m/Y'); ?></td>
