  ...
  <table class="table table-striped-2">
    <thead>
      <tr>
        ...
        <th class="thin-cell text-right" nowrap>ยอดกดรับ</th>
        <th class="thin-cell text-right" nowrap>ส่วนต่างกระเป๋าเงิน</th>
      </tr>
    </thead>
    <tbody>
      <?php
      foreach ($select_summary_credit_transaction as $value) {
        if ($value['date'] == 'ยอดรวม') {
          $total_kod_rub = isset($value['sum_kod_rub']) ? $value['sum_kod_rub'] : 0;
          $total_difference = isset($value['difference']) ? $value['difference'] : 0;
      ?>
          <tr>
            ...
            <td nowrap class="text-right bg-blue-2 font-SemiBold"><?= number_format($total_kod_rub, 2) ?></td>
        ...
        <?php } else {
          $sum_kod_rub = isset($value['sum_kod_rub']) ? $value['sum_kod_rub'] : 0;
          $difference = isset($value['difference']) ? $value['difference'] : 0;
        ?>
          <tr>
            ...
            <td nowrap class="text-right font-SemiBold"><?= number_format($sum_kod_rub, 2) ?></td>
