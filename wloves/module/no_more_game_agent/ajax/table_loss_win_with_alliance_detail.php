...
$where = [
  ...
  'user_register_count' => isset($_POST['user_register_count']) ? $_POST['user_register_count'] : '',
  'user_count_by_first_time_date' => isset($_POST['user_count_by_first_time_date']) ? $_POST['user_count_by_first_time_date'] : '',
...
$options = [
  ...
  'sort'        => isset($_POST['data_sort']) && $_POST['data_sort'] ? $_POST['data_sort'] : ['deposit_first_time_date' => 'DESC']
];
...
<tbody data-total_count="<?= $total_count; ?>">
  <?php foreach ($list as $data) { ?>
    <tr class="tr-link cursor-pointer" data-link="loss_win_with_alliance_description_detail.php?c=<?= $code ?>&id=<?= $data['id'] ?>&date_data=<?= $data['deposit_first_time_date'] ?>">
      <td nowrap>
        <div>
          <?php
      ...
      <td nowrap>
        <div class="text-primary text-right"><?= number_format($data['user_register_count'], 0); ?></div>
      </td>
      <td nowrap class="text-success text-right">
        <?= isset($data['user_count_by_first_time_date']) ? number_format($data['user_count_by_first_time_date']) : ''; ?>
      </td>
