...
<tbody data-total_count="<?= $total_count ?>">
  <?php foreach ($list as $key => $invite_list) {
    ...
    if ($invite_list['upline2_user_id'] && $invite_list['upline2_user_id'] == $customer_id) {
      $mlm = '(สายชั้นที่ 2)';
    } else if ($invite_list['upline3_user_id'] && $invite_list['upline3_user_id'] == $customer_id) {
      $mlm = '(สายชั้นที่ 3)';
    } else {
      $mlm = '(สายชั้นที่ 1)';
    }
  ?>
    <tr class="tr-link cursor-pointer <?= $table_active; ?>" data-link="?c=<?= $code ?>&id=<?= $customer_id ?>&page=12&friend_id=<?= $invite_list['id'] ?>&upline_id=<?= $customer_id ?>">
      ...
      <td nowrap class="text-right vertical-center my-auto">
        <div class="text-vertical-center pt-8px">
          <?= number_format($invite_list['sum_upline_receive_new'], 2); ?>
        </div>
      </td>
      <td nowrap class="vertical-center text-center">
        <?php if ($invite_list['receive_new_status'] == 'waiting') { ?>
          <div class="text-vertical-center pt-8px text-orange">
            รอรับ
          </div>
        <?php } else if ($invite_list['receive_new_status'] == 'completed') { ?>
