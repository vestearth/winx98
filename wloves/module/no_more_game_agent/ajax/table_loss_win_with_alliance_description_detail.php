...
<tbody data-total_count="<?= $total_count; ?>">
  <tr>
    ...
    <td></td>
    <td class="text-right font-Bold font-16px text-primary"><?= number_format($list[0]['sum_count_deposit_time'], 0) ?></td>
    <td class="text-right font-Bold font-16px text-primary"><?= number_format($list[0]['sum_all_deposit'], 2) ?></td>
    <td class="text-right font-Bold font-16px <?= ($list[0]['sum_all_win_lost'] > 0) ? 'text-primary' : 'text-danger' ?>"><?= number_format($list[0]['sum_all_win_lost'], 2) ?></td>
    <td></td>
  </tr>
    ...
    <tr class="tr-link cursor-pointer" data-link="customer_details.php?c=<?= $code ?>&id=<?= $ally_data['id'] ?>&page=6">
      ...
      <td nowrap>
        <?php if ($ally_data['first_time_deposit_date']) { ?>
          <?= Aww::formatDate($ally_data['first_time_deposit_date'], 'd/m/Y'); ?>
        <?php } else { ?>
          <span>-</span>
        <?php } ?>
      </td>
