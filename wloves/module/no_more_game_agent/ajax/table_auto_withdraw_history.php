  ...
  <?php foreach ($data_list as $bot_statement) { ?>
    <tr>
      <td nowrap>
        ...
        <div class="text-blue">
          <?php
          if ($bot_statement['confirm_transaction_date_time']) {
            echo Aww::formatDate($bot_statement['confirm_transaction_date_time'], 'd/m/Y, H:i');
          }
          if ($bot_statement['cancel_transaction_date_time']) {
            echo Aww::formatDate($bot_statement['cancel_transaction_date_time'], 'd/m/Y, H:i');
          }
          ?>
        </div>
      </td>
      ...
      <td nowrap class="thin-cell">
        <div class="form-row ">
          <div class="col-lg-12">
            <div class="form-row">
              <?php if ($bot_statement['status'] == 'completed' || $bot_statement['is_can_withdraw_over_limit'] == 1) { ?>
      ...
      <td class="text-nowrap"><?= $bot_statement['cancel_remark']; ?></td>
      <td class="cursor-pointer">
        <?php if ($bot_statement['status'] == 'completed') { ?>
          <div class="w-50px" <?php Tiwdal::register('modal_bot_statement_detail', $bot_statement); ?>>
            <img class="w-100 border-radius-10px" src="<?= $bot_statement['admin_confirm_image'] ?>">
          </div>
        <?php } ?>
      </td>
    </tr>
